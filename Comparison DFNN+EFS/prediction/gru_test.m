...
data_gen;
net=[];
lr=[0.01 0.03 0.03 0.1 0.1 0.02];
load GRU_net;
for k=1:1
    x=x_train(1:10000,:)';
    y=y_train(1:10000,k)';
    ...
    %% 训练神经网络参数设定
    numHiddenUnits =50;    %指定LSTM层的隐含单元个数为50
    ...
    layers = [ ...
        ...
        gruLayer(numHiddenUnits,'Outputmode','sequence','name','hidden')
        reluLayer('name','relu')
    ...
    options = trainingOptions('adam', ... %指定训练选项，求解器设置为adam， 1000轮训练。
        'MaxEpochs',500, ...    %最大训练周期为150
        'Minibatch',32, ...    %最大训练周期为150
        'ExecutionEnvironment','gpu',...
        'L2Regularization',0.001,... 
        'InitialLearnRate',lr(k), ...  %指定初始学习率 0.01
        'LearnRateSchedule','piecewise', ...  %每当经过一定数量的时期时，学习率就会乘以一个系数。
        'LearnRateDropPeriod', 50, ...
        'LearnRateDropFactor',1, ...  %在50轮训练后通过乘以因子 0.5 来降低学习率。
...
save('GRU_net', 'net');
%%
taulstm=zeros(size(y_test'));
for k=1:6
    net{k} = resetState(net{k});

    % yp=net.predict(x_train')';
    for i = 2:size(x_test',2)
        [net{k},taulstm(k,i)] = predictAndUpdateState(net{k},x_test(i-1,:)','ExecutionEnvironment','gpu');  %
    end
    figure();hold on;
    plot(taulstm(k,:)');plot(y_test(:,k));
    
end
rmsen(taulstm',y_test)
